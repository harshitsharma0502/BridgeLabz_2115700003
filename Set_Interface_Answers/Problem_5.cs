...
class Program
{
    static void Main()
    {
        ...
        HashSet<int> subset = new HashSet<int>(Console.ReadLine().Split(' ').Select(x => Convert.ToInt32(x)));

        Console.Write("Enter elements of Main Set separated by space: ");
        HashSet<int> mainSet = new HashSet<int>(Console.ReadLine().Split(' ').Select(x => Convert.ToInt32(x)));
